...
describe('MailhooksTrigger — webhook attachment download URLs', () => {
	...
	it('should include storagePath on attachments when provided (custom storage)', async () => {
		const node = new MailhooksTrigger();
					...
					attachments: [
						{
							...
							contentType: 'application/pdf',
							size: 2048,
							storagePath: 's3://my-bucket/emails/email-3/report.pdf',
						},
					],
					receivedAt: '2026-04-18T17:00:00Z',
					usesCustomStorage: true,
				},
				headers: {},
			})),
			getWorkflowStaticData: jest.fn(() => ({})),
		...
		expect(attachments).toHaveLength(1);
		expect(attachments[0].storagePath).toBe('s3://my-bucket/emails/email-3/report.pdf');
		expect(attachments[0].downloadUrl).toBeUndefined();
	});

	it('should omit downloadUrlExpiresAt when only downloadUrl is present', async () => {
		const node = new MailhooksTrigger();
					...
					attachments: [
						{
							...
							size: 512,
							downloadUrl: 'https://mailhooks.dev/api/v1/emails/email-4/attachments/att-4/download?token=signed',
						},
					],
					receivedAt: '2026-04-18T17:00:00Z',
					usesCustomStorage: false,
		...
		expect(attachments).toHaveLength(1);
		expect(attachments[0].downloadUrl).toBe('https://mailhooks.dev/api/v1/emails/email-4/attachments/att-4/download?token=signed');
		expect(attachments[0].downloadUrlExpiresAt).toBeUndefined();
	});
});